...
- `版权归属` 要填写版权归属主体名称，不要只写 `原创`。如果主体名不确定，提交前先问清楚。
- `角色/内容` 要按画面主体选，不按绘画风格选。真人或真人参考图生成的女性形象，优先按平台口径选 `人物角色 - 女人`，不要因为画风卡通就选 `动漫/漫画/卡通人物`。
- 如果勾选 `接受赞赏`，必须同时补齐赞赏引导语、赞赏引导图和赞赏致谢图；否则保存或提交时会卡校验。
- 如果审核驳回，先读驳回页面的原文，只修被点名的字段，保存确认预览信息更新后再重新提交。
